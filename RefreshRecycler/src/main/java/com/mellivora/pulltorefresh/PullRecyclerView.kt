...
class PullRecyclerView(context: Context, attrs: AttributeSet? = null) : FrameLayout(context, attrs), RefreshResultInterface {
    ...
    private val statusView: MultipleStatusView
    ...
    private fun onSizeChange(count: Int, emptyView: View?, isAdapterEmpty: Boolean) {
        ...
        if (count > 0) {
            ...
            if (emptyView != null) {
                statusView.showEmpty(emptyView, ViewGroup.LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT))
            } else {
    ...
    fun setAdapter(adapter: RecyclerView.Adapter<*>?, emptyView: View? = null, isAdapterEmpty: Boolean = false) {
        onSizeChange(adapter?.itemCount ?: 0, emptyView, isAdapterEmpty)
        ...
        if (adapter != null) {
            pullAdapterObserver = object : RecyclerView.AdapterDataObserver() {
                override fun onChanged() {
                    onSizeChange(adapter.itemCount, emptyView, isAdapterEmpty)
                }
     ...
     * 调用前记得取消相关请求任务
     */
    fun showLoading(notify: Boolean = true) {
    ...
    override fun loadFinish(isRefresh: Boolean, hasMore: Boolean, footerDivider: Boolean) {
        ...
        if(!hasMore){
            if(page <= 1) return
            if(footerLineView == null) footerLineView = LayoutInflater.from(context).inflate(R.layout.footer_line, swipeRecyclerView, false)
